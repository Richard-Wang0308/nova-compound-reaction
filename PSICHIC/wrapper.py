...
class PsichicWrapper:
    def __init__(self, model_path: str = None):
        ...
        Args:
            model_path: Path to the TREAT model directory (TREAT1 or TREAT2).
                       If None, uses default from RuntimeConfig (TREAT1).
        """
        ...
        if model_path is None:
            self.model_path = self.runtime_config.MODEL_PATH
        else:
    ...
    def load_model(self):
        degree_dict = torch.load(os.path.join(self.model_path,
                                              'degree.pt'), 
                                 weights_only=True
                                 )
        param_dict = os.path.join(self.model_path, 'model.pt')
